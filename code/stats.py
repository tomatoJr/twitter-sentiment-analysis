...
if __name__ == '__main__':
    ...
    all_words = []
    all_bigrams = []
    processed_file_name = CSV_PROCESSED_PATH + sys.argv[1]
    with open(processed_file_name, 'r', encoding="ISO-8859-1") as csv:
        lines = csv.readlines()
        num_tweets = len(lines)
        for i, line in enumerate(lines):
            t_id, tag, tweet = line.strip().split(',')
            tag = int(tag)
            if tag == 4:
                num_pos_tweets += 1
            elif tag == 0:
    ...
    with open(CSV_PROCESSED_PATH + sys.argv[1][:-4] + '-unique.txt', 'w') as uwf:
        uwf.write('\n'.join(unique_words))
    ...
    with open(CSV_PROCESSED_PATH + pkl_file_name, 'wb') as pkl_file:
    ...
    with open(CSV_PROCESSED_PATH + bi_pkl_file_name, 'wb') as pkl_file:
